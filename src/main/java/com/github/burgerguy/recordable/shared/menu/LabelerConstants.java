package com.github.burgerguy.recordable.shared.menu;

import net.minecraft.world.item.DyeColor;

public class LabelerConstants {
    public static final int CONTAINER_SIZE = 4;

    public static final int PALETTE_X = 10;
    public static final int PALETTE_Y = 30;
    public static final int PALETTE_COLUMNS_WRAP = 4;
    public static final int COLOR_WIDTH = 8;
    public static final int COLOR_HEIGHT = 20;
    public static final int COLOR_MARGIN_X = 8;
    public static final int COLOR_MARGIN_Y = 8;

    public static final int DEFAULT_BORDER_COLOR = 0xFF000000;
    public static final int SELECTED_BORDER_COLOR = 0xFFFF0000;

    public static final int COLOR_MAX_CAPACITY = 16;
    public static final int COLOR_CAPACITY_PER_ITEM = 4;

    public static final int COLOR_COUNT = DyeColor.values().length;
}
